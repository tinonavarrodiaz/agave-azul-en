const slidesImg = ["Landscape-m", "AgaveAzul-m", "Inicio3-m", "Inicio4-m"];
...
export const sliderTequilas = (gallery, isMobile) => {
  ...
  const maxPos = isMobile ? widthItem * 7 : widthItem * 4;
  // console.log(widthItem, maxPos)
  next.addEventListener("click", e => {
    let pos = parseInt(
      getComputedStyle(dd).getPropertyValue("--tequilasSliderPos")
    );
    e.preventDefault();
    if (pos > maxPos * -1) {
